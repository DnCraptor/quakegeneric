...
// Pins used for the mouse
static uint8_t mouse_clk_pin, mouse_data_pin;

// Forward declarations
static void mouse_buffer_flush();
...
//-----------------------------------------------------------------------------
// Send a byte to the mouse (host-to-device). Flushes receive buffer
//-----------------------------------------------------------------------------
...
static void mouse_buffer_flush() {
    uint8_t dummy;
    while (mouse_buffer_get(&dummy)) { tight_loop_contents(); }
}
...
int ps2mouse_init(int pin_clk, int pin_data) {
    uint8_t dummy;
    ...
    mouse_clk_pin  = pin_clk;
    mouse_data_pin = pin_data;
    ...
    // Check for a response whthin 500ms - if there is none, assume mouse is not present
    {
        int      got_response;
        uint64_t deadline = time_us_64() + 500*1000;
        while ((time_us_64() < deadline) && ((got_response = mouse_buffer_get(&dummy)) == 0)); {
            tight_loop_contents();
        }
        if (got_response == 0) return 0;
    }
    
    // Clear other response bytes if any
...
int ps2mouse_get_state(int16_t *dx, int16_t *dy, int8_t *wheel, uint8_t *buttons) {
    ...
    int has_data = (mouse_state.delta_x != 0 || 
                    ...
                    mouse_state.wheel   != 0 ||
                    mouse_state.buttons != mouse_state.buttons_old);
    ...
    mouse_state.delta_y = 0;
    mouse_state.wheel = 0;
    mouse_state.buttons_old = mouse_state.buttons;
