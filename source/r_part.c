...
avelocities[0][i] = (rand()&255) * 0.01f;
	...
	{
		angle = cltime_f * avelocities[i][0];
		sy = sinf(angle);
		cy = cosf(angle);
		angle = cltime_f * avelocities[i][1];
		sp = sinf(angle);
		cp = cosf(angle);
		angle = cltime_f * avelocities[i][2];
		sr = sinf(angle);
		cr = cosf(angle);
...
void R_ReadPointFile_f (void)
{
	...
	snprintf (name, MAX_OSPATH, "maps/%s.pts", svp.name);
