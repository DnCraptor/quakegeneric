...
{
	static void* ledges = 0;
	static void* lsurfs = 0;
	if (!ledges) {
		ledges = alloc((NUMSTACKEDGES + ((CACHE_SIZE - 1) / sizeof(edge_t)) + 1) * sizeof(edge_t), "ledges");
		lsurfs = alloc((NUMSTACKSURFACES +	((CACHE_SIZE - 1) / sizeof(surf_t)) + 1) * sizeof(surf_t), "lsurfs");
	}
